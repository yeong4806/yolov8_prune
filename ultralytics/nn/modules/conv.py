...
class Conv(nn.Module):
    ...
    def recon(self, remain_in_channels, remain_out_channels=None):
        # conv weight 0이 아닌 index 찾기
        # conv weight 수정
        # conv out_channels
        # ocnv in_chnanels
        # batch norm 5개
        if remain_out_channels == None:
            ...
            self.conv.weight = torch.nn.parameter.Parameter(torch.index_select(self.conv.weight, 1, torch.tensor(remain_in_channels)))
            ...
            self.conv.weight = torch.nn.parameter.Parameter(
                torch.index_select(self.conv.weight, 1, torch.tensor(remain_in_channels)))
            self.conv.weight = torch.nn.parameter.Parameter(

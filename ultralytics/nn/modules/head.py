...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=(), cv3_inchannels=64, cv3_outchannels=64):
        ...
        if cv3_inchannels == cv3_outchannels:
    ...
    def recon(self, detect_in_channels):
        ...
        for i, detect_in in enumerate(detect_in_channels):
            self.cv3[i][0].conv.weight = torch.nn.parameter.Parameter(
                        torch.index_select(self.cv3[i][0].conv.weight, 1, torch.tensor(detect_in)))
            self.cv3[i][0].conv.in_channels = len(detect_in)
